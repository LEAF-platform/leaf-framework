...
def create_dashboard_panel(self, dashboard_tab) -> None:
        ...
        with ui.column().classes('w-1/2'):
            with ui.card().classes('w-full'):
                ...
                with ui.row():
                    ...
                    ui.label().bind_text_from(self.leaf_state, 'active_adapters')
            ...
            with ui.row().classes('w-full'):
                ...
                with ui.card().classes('w-full'):
                    ...
                    def update_errors() -> None:
                        ...
                        if not self.leaf_state['errors']:
                            with error_list:
                                dt = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                ui.item(f'No errors ({dt})').classes('text-gray-500')
                        else:
                            with error_list:
                                for error in self.leaf_state['errors']:
                ...
                with ui.card().classes('w-full'):
                    ...
                    def update_warnings() -> None:
                        ...
                        # Add errors or "No errors" message
                        if not self.leaf_state['warnings']:
                            with error_list:
                                dt = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                ui.item(f'No warnings ({dt})').classes('text-gray-500')
                        else:
                            with error_list:
                                for error in self.leaf_state['warnings']:
                                    ui.item(error).classes('text-red-500 text-sm')
