...
def create_config_panel(tabs, config_tab, self) -> None:
    ...
    with ui.tab_panel(config_tab):
        ...
        # Two columns using nicegui grid, left column contains the form and right column contains the output
        with ui.row().style("display: flex; width: 100%"):
            with ui.column().style("width: 40%"):
                # Configuration form
                config_path = ui.input(label='Configuration File Path!!', value=self.global_args.config)
                ...
                def load_config() -> None:
                    ...
                    try:
                        with open(path, 'r') as file:
                            ...
                            config_editor.value = yaml.dump(self.global_config, indent=4).replace('\n', '<br>').replace(' ', '&nbsp;')
                ...
                def save_config() -> None:
                    ...
                    try:
                        with open(path, 'w') as file:
                            file.write(config_editor.value.replace('<br>', '\n').replace('&nbsp;', ' '))
