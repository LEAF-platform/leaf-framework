...
#             from leaf.start import context
#             # if context.config is not None and self.global_args.config is not None:
#                 # with open(self.global_args.config, 'r') as file:
#                     # self.global_config = yaml.safe_load(file)
#             # if self.global_config is None:
#             config_editor = ui.codemirror(value=context.config_yaml,language="YAML").style('width: 100%')
#             # else:
...
#             async def save_config(self, context: AppContext, config_content: str) -> None:
...
#                 #     with open(path, 'r') as file:
#                     context.config = yaml.safe_load(config_content) # yaml.safe_load(file)
