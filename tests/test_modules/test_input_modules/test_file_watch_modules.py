...
class TestFileWatcher(unittest.TestCase):
    def test_file_watcher_change(self):
        with tempfile.TemporaryDirectory() as test_dir:
            ...
            interval = 2
            watcher = FileWatcher(
                text_watch_file, MetadataManager(), measurement_callbacks=mock_callback
            )
            watcher.start()
    ...
    def test_file_watcher_creation(self):
        with tempfile.TemporaryDirectory() as test_dir:
            ...
            watcher = FileWatcher(
                creation_file, MetadataManager(), 
                start_callbacks=mock_callback)
            num_create = 3
            interval = 2
            watcher.start()
    ...
    def test_file_watcher_deletion(self):
        with tempfile.TemporaryDirectory() as test_dir:
            ...
            watcher = FileWatcher(
                deletion_file, MetadataManager(), stop_callbacks=mock_callback
            )
            num_create = 3
            interval = 2
            watcher.start()
